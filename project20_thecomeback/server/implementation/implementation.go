...
func (server *Server_Struct) SendFile(stream pb.File_Transfer_SendFileServer) error {
	...
	*/
	...
	// select {
	// case <-done_status:
	// 	fs, _ := os.OpenFile(name_of_file, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	...
	go func() {
		for packet := range packet_chan {
			mutex.Lock()
			if name_of_file == "" {
				name_of_file = fmt.Sprintf("%v/%v", server.Dir, packet.Name)
				fs, _ = os.Create(name_of_file)
			}

			// fmt.Println(string(packet.GetData()))

			mutex.Unlock()
			name_of_chunk := fmt.Sprintf("%v/%v", server.Dir, strings.Split(packet.Name, ".")[0]+strings.Split(packet.Name, ".")[1])
			_ = os.WriteFile(name_of_chunk, packet.GetData(), 0644)
			data, _ := os.ReadFile(name_of_chunk)

			fs.Write(data)

			os.Remove(name_of_chunk)
